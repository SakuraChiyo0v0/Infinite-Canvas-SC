(function(){
    ...
    function canLeave(){return !editing || confirm('尚未保存角色资料，放弃这次修改？');}
    async function load(){
        status('正在加载…');
    ...
    function renderTemplates(){
        const previous=$('characterTemplate').value || remembered.template || '';
        $('characterTemplate').innerHTML='<option value="">选择一个提示词模板</option>'+libraries.map(lib=>`<optgroup label="${escape(lib.name)}">${lib.items.map(item=>`<option value="${escape(JSON.stringify([lib.id,item.id]))}">${escape(item.name)}</option>`).join('')}</optgroup>`).join('');
        if([...$('characterTemplate').options].some(o=>o.value===previous))$('characterTemplate').value=previous;
    }
    function renderList(){
    ...
    $('characterNew').onclick=()=>{if(uploading||saving||!canLeave())return;editing={name:'',description:'',references:[],cover:'',revision:0};renderProfile();};
    $('characterRefresh').onclick=()=>{if(uploading||saving||!canLeave())return;editing=null;load().catch(e=>status(e.message));};
    ...
    document.addEventListener('submit',async event=>{
        ...
        try{const data=await json('/api/characters'+(editing.id?'/'+editing.id:''),{...body(editing),method:editing.id?'PUT':'POST'});selected=data.character.id;editing=null;await load();workbench.updateCharacter(data.character);status('角色已保存，选择模板即可创作');}
    ...
    document.addEventListener('click',async event=>{
        ...
        if(entry){if(uploading||saving||!canLeave())return;selected=entry.dataset.character;editing=null;render();}
        if(t.closest('[data-character-edit]')){editing=core.clone(profile());renderProfile();}
        if(t.closest('[data-character-cancel]') && !uploading && !saving && canLeave()){editing=null;renderProfile();}
