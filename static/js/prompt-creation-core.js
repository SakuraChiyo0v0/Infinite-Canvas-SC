(function(global){
    ...
    function safeUrl(url){return /^(\/assets\/|\/output\/|https?:\/\/)/i.test(String(url || ''))?String(url):'';}
    function provenance(result){return clone({version:1,resultId:result.id,parentResultId:result.parentResultId || '',prompt:result.prompt,model:result.model,provider:result.provider || '',size:result.size,operation:result.operation,references:result.references || [],character:result.character || null,source:result.source || null,instruction:result.instruction || '',createdAt:result.createdAt || 0});}
