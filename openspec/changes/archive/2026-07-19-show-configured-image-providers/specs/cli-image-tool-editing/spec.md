...
The detail enhancement, image editing, and angle-control screens SHALL expose a remote engine source alongside their existing engines. Each screen MUST list every enabled API provider with at least one configured image model, including CLI, custom API, and built-in providers when they meet those availability conditions, and refresh that list when provider settings change.

#### Scenario: Eligible providers are configured
- **WHEN** a user opens any of the three local image tools and API settings contain eligible GPT CLI or 鍗虫ⅵ CLI image providers
...
- **WHEN** a user selects the remote engine in one of the local image tools and no enabled image provider is configured
